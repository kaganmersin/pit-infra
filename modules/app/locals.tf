locals {
  app_service_name      = join(var.delimiter, compact([local.stage_prefix, var.prefix, module.azure_region.location_short, var.name]))
  app_service_slot_name = join(var.delimiter, compact([local.app_service_name, var.staging_slot_name_abbreviation]))
  default_site_config = {
    always_on               = "true"
