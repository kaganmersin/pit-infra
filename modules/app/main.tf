...
resource "azurerm_windows_web_app" "default" {
    ...
    content {
      ...
      dynamic "active_directory_v2" {
        ...
        content {
          ...
          allowed_audiences                    = concat(formatlist("https://%s", [format("%s.azurewebsites.net", local.app_service_name)]), lookup(active_directory_v2.value, "allowed_audiences", []))
        }
      }
  ...
  dynamic "backup" {
    ...
    content {
      name                = "${local.app_service_name}-backup"
...
resource "azurerm_windows_web_app_slot" "default" {
  count = var.create && var.staging_slot_enabled ? 1 : 0
  ...
  public_network_access_enabled = var.public_network_access_enabled
  virtual_network_subnet_id     = var.app_service_vnet_integration_subnet_id
  app_settings                  = var.staging_slot_custom_app_settings == null ? var.app_settings : var.staging_slot_custom_app_settings
...
resource "azurerm_app_service_custom_hostname_binding" "default" {
  ...
  hostname            = each.key
  app_service_name    = azurerm_windows_web_app.default.name
  resource_group_name = var.resource_group_name
  ...
  thumbprint          = lookup(each.value, "certificate_thumbprint", null) != null ? each.value.certificate_thumbprint : lookup(each.value, "certificate_name", null) != null ? azurerm_app_service_certificate.default[each.value.certificate_name].thumbprint : null
  lifecycle {
