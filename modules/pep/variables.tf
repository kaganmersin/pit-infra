...
variable "terraform_module" {
  type        = string
  default     = "kaganmersin/pit-infra/modules/pep"
...
locals {
  ...
  private_dns_zone_group_name     = local.is_not_private_link_service ? join(var.delimiter, [local.module_prefix, module.azure_region.location_cli, "pdnszg"]) : null
  private_service_connection_name = join(var.delimiter, [local.module_prefix, module.azure_region.location_cli, "psc"])
}
